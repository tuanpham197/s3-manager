...
export default function VideoRow({ video }: VideoRowProps) {
  ...
  const handleCopy = () => {
    navigator.clipboard.writeText(video.s3Link).catch(() => {});
  };
    ...
    <tr className="group hover:bg-[#F8FAFC] transition-colors duration-150">
      ...
      <td className="px-6 py-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <p className="text-sm font-semibold text-[#1E293B] leading-tight">{video.name}</p>
      ...
      <td className="px-6 py-4">
        {isUploading ? (
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] font-bold text-[#DC2626] uppercase tracking-tight">
              Failed
            </span>
          </div>
        ) : (
      ...
      <td className="px-6 py-4 text-right">
        <div className="flex items-center justify-end gap-1">
          ...
          <button
            className="p-2 text-[#94A3B8] hover:text-[#1E293B] hover:bg-[#F1F5F9] rounded-lg transition-all duration-150 cursor-pointer"
